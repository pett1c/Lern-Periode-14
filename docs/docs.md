...
## MVP Contract (Current Frontend)

Das aktuelle Frontend nutzt derzeit folgende API-Endpunkte:
...
- Public Registration akzeptiert nur die Rollen `user` und `organizer`.
- Rolle `admin` ist im Public Register bewusst gesperrt.
...
```json
{
  ...
  "error": {
    ...
    "details": [
      { "path": "field", "message": "Description of the issue." }
    ]
  }
}
```
